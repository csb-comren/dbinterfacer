...
select = qu.get_select_string(['points', 'point_data'], ['depth < 2'], ['depth', 'latitude', 'geom'])
# print(select)
# print(len(list(qu.get_depth_points_json(local_url, ['depth > -1']))))

json_q = qu.get_json(local_url, select, [("'type'", "'Feature'"), ("'depth'", 'depth'), ("'geometry'", "geom")])
print(qu.query(local_url, json_q))
